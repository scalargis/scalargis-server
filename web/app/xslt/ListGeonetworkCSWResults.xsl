  ...
  <xsl:template match="gmd:identificationInfo">
    ...
    <xsl:if test="srv:SV_ServiceIdentification">
      ...
      <div>
        <div class="col-md-4">
          <a target="_blank" class="metadataLink">
            <xsl:attribute name="href"><xsl:value-of select="$urlBase"/>/<xsl:value-of select="../gmd:fileIdentifier/gco:CharacterString" /></xsl:attribute>
            <xsl:value-of select="$labelMetadados" />
          </a>                
        </div>
        <div class="col-md-4">
          <a target="_blank" class="metadataLink">
            <xsl:attribute name="href">http://idealg.ccdr-alg.pt/catalogo/srv/api/records/<xsl:value-of select="../gmd:fileIdentifier/gco:CharacterString" />/formatters/xml</xsl:attribute>
            <xsl:value-of select="$labelXML" />
          </a>        
        </div>
        <div class="col-md-offset-1 col-md-3">
