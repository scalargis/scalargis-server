    ...
    <tr>
      <td style="break-word; overflow-wrap: break-word;">
        ...
        <div>
          ...
          <div class="col-md-4">
			<xsl:choose>
				<xsl:when test="(./dc:metadataDocUrl[contains(@scheme,'Document')])">
					<a target="_blank" class="metadataLink">
					  <xsl:attribute name="href">
						<xsl:value-of select="./dc:metadataDocUrl[contains(@scheme,'Document')]"/>
					  </xsl:attribute>
					  <xsl:value-of select="$labelXML" />
					</a>
				</xsl:when>
				<xsl:when test="(./dct:references[contains(@scheme,'Document')])">
					<a target="_blank" class="metadataLink">
					  <xsl:attribute name="href">
          ...
          <div class="col-md-offset-1 col-md-3">
            <xsl:choose>
			  ...
			  <xsl:when test="(./dc:subject='infoMapAccessService') and (./dc:resourceUrl[contains(@scheme,'Server')])">
				<a href="#">
				  <xsl:attribute name="data-wms-link">
						<xsl:value-of select="./dc:resourceUrl[contains(@scheme,'Server')]"/>
				  </xsl:attribute>
				  <xsl:value-of select="$labelAddService" />
				</a>
			  </xsl:when>			  
