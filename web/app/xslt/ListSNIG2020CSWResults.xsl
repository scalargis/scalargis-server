...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
 ...
 xmlns:csw="http://www.opengis.net/cat/csw/2.0.2">
  ...
  <xsl:template match="gmd:identificationInfo">
      ...
      <div>
        <div class="col-md-4">
          <a target="_blank" class="metadataLink">
            <xsl:attribute name="href"><xsl:value-of select="$urlBase"/>/srv/api/records/<xsl:value-of select="../gmd:fileIdentifier/gco:CharacterString" />/formatters/snig-view</xsl:attribute>
            <xsl:value-of select="$labelMetadados" />
          </a>                
        </div>
        <div class="col-md-4">
          <a target="_blank" class="metadataLink">
            <xsl:attribute name="href"><xsl:value-of select="$urlBase"/>/srv/api/records/<xsl:value-of select="../gmd:fileIdentifier/gco:CharacterString" />/formatters/xml</xsl:attribute>
    ...
    <xsl:if test="srv:SV_ServiceIdentification|SV_ServiceIdentification">
      <a href="#" class="a-record">
        <xsl:attribute name="fid">
          <xsl:value-of select="../gmd:fileIdentifier/gco:CharacterString" />
        </xsl:attribute>
        <xsl:attribute name="title">
          <xsl:apply-templates select="(./srv:SV_ServiceIdentification|./SV_ServiceIdentification)/gmd:citation"/>
        </xsl:attribute>        
        <xsl:if test="(./srv:SV_ServiceIdentification|./SV_ServiceIdentification)/srv:extent/gmd:EX_Extent">
          <xsl:apply-templates select="(./srv:SV_ServiceIdentification|./SV_ServiceIdentification)/srv:extent/gmd:EX_Extent"/>
        </xsl:if>
        <xsl:apply-templates select="(./srv:SV_ServiceIdentification|./SV_ServiceIdentification)/gmd:citation"/>
      </a>  
      
      <p class="muted top-margin-10">
        <xsl:value-of select="substring((./srv:SV_ServiceIdentification|./SV_ServiceIdentification)/gmd:abstract/gco:CharacterString, 1, 250)" />
        <xsl:if test="string-length((./srv:SV_ServiceIdentification|./SV_ServiceIdentification)/gmd:abstract/gco:CharacterString) > 250" >
          <xsl:text> ...</xsl:text>
        </xsl:if>
      </p>   
      ...
      <div>
        ...
        <div class="col-md-offset-1 col-md-3">
          <xsl:if test="(./srv:SV_ServiceIdentification|./SV_ServiceIdentification)/srv:containsOperations/srv:SV_OperationMetadata/srv:connectPoint/gmd:CI_OnlineResource/gmd:linkage">
            <xsl:text> </xsl:text>
            <a href="#" class="metadataLink">
              <xsl:attribute name="data-wms-link">
                <xsl:value-of select="(./srv:SV_ServiceIdentification|./SV_ServiceIdentification)/srv:containsOperations/srv:SV_OperationMetadata/srv:connectPoint/gmd:CI_OnlineResource/gmd:linkage"/>
              </xsl:attribute>
              <xsl:value-of select="$labelAddService" />
            </a>
          </xsl:if>
