...
app.Popup = function (options) {
	...
	$('<div id="popup-title"></div>').appendTo($(this.getElement())).html(options.title || 'Informação');
	$('<div id="popup-content"></div>').appendTo($(this.getElement())).html(options.conten || '');
};

ol.inherits(app.Popup, ol.Overlay);

/**
 * Set the title to be shown in the popup.
 * @param {string} title The title to be shown.
 */
app.Popup.prototype.setTitle = function (title) {
	document.getElementById('popup-title').innerHTML = content;
};
