...
def init_email(app, security):
    """
    Performs email init.

    Args:
        app: flask application
        security: flask security context
    """

    @security.send_mail_task
    def async_security_email(msg):
        send_security_email(app, msg)
...
def init_ldap(app):
    ...
    if 'LDAP_AUTHENTICATION' in app.config and app.config.get('LDAP_AUTHENTICATION') is True:
        ...
        app.config['LDAP_AUTHENTICATION'] = False
...
def authenticate_ldap_user(username, password, domain):
    ...
    if current_app.config.get('LDAP_AUTHENTICATION') is True:
...
@async_task
def send_security_email(app, msg):
    """ Sends an send_email asynchronously
    Args:
        app (flask.Flask): Current flask instance
